...
public:
    ...
    typedef map<string, Entry*> NpaRegistry;
    typedef NpaRegistry::iterator NpaIterator;

    INpaFile(const string& Filename) : NpaFile(Filename) { }
    virtual ~INpaFile() = 0;
    ...
    NpaIterator FindFile(const string& Filename) { return Registry.find(Filename); }
